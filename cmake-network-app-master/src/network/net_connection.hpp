...
#ifndef NET_MAX_BODY_SIZE
#define NET_MAX_BODY_SIZE 256
#endif // !NET_MAX_BODY_SIZE
...
namespace net
{
	...
	{
	public:
		// A connection is "owned" by either a server or a client, and its behaviour is slightly different bewteen the two.
		enum class owner
		{
			server,
			client
			...
			if (m_nOwnerType == owner::client)
			{
				...
				asio::async_connect(m_socket, endpoints,
									[this](std::error_code ec, asio::ip::tcp::endpoint endpoint)
									{
										...
										{
											std::cout << "Failed to connect to server: " << endpoint << std::endl;
										}
									});
			}
		}
	...
	private:
		...
		void ReadHeader()
		{
			...
			asio::async_read(m_socket, asio::buffer(&m_msgTemporaryIn.header, sizeof(message_header<T>)),
							 [this](std::error_code ec, std::size_t length)
							 {
								 if (!ec)
								 {
									 if (m_msgTemporaryIn.header.size > NET_MAX_BODY_SIZE)
									 {
										 // INVALIDATE CLIENT
										 std::cout << "[" << id << "] Invalid body size\n";
										 Disconnect();
									 }
									 // A complete message header has been read, check if this message
									 // has a body to follow...
									 else if (m_msgTemporaryIn.header.size > 0)
									 {
		...
		// And start listening for a header again
